...
function ChangeSteamWidget() {
    document.getElementById("SteamWidget").style.display = "block";
}

function RemoveSteamWidget() {
    document.getElementById("SteamWidget").style.display = "none";
}
...
function steamGames() {
    var game_name = document.getElementById("GameInput");
    var player_count = callSteam(game_name);
    document.getElementById("playerCount").innerHTML = "Current players :" + player_count;
}

async function callSteam(gamename) {
    let appId = '';

    /* requires axios, a database extractor */
    const axios = require('axios');

    /* gets and parse the Steam db into response */
    axios.get('http://api.steampowered.com/ISteamApps/GetAppList/v0002/')
    .then(async function (response) {
        console.log(user);

        /* Takes the amount of players in the steam Database from a game's name */
        appId = retrievePlayerCount(response.data.applist, gamename);
        appId = appId.appid.toString();

        /* gets and parse the steam player count of a game*/
        axios.get('https://api.steampowered.com/ISteamUserStats/GetNumberOfCurrentPlayers/v1/?format=json&appid=' + appId);
        return response.player_count;
    });
}

function retrievePlayerCount(applist, gamename) {
